...
use super::postcss::PostCssConfigLocation;

#[turbo_tasks::value(shared)]
#[derive(Clone, Default)]
pub struct WebWorkerPostCssTransformOptions {
    pub config_location: PostCssConfigLocation,
    pub placeholder_for_future_extensions: u8,
}

#[turbo_tasks::value]
pub struct WebWorkerPostCssTransform {
...
impl WebWorkerPostCssTransform {
...
#[turbo_tasks::value_impl]
impl SourceTransform for WebWorkerPostCssTransform {
    #[turbo_tasks::function]
    async fn transform(&self, source: Vc<Box<dyn Source>>) -> Result<Vc<Box<dyn Source>>> {
        Ok(Vc::upcast(
            WebWorkerPostCssTransformedAsset {
...
#[turbo_tasks::value]
pub struct WebWorkerPostCssTransformedAsset {
...
#[turbo_tasks::value_impl]
impl Source for WebWorkerPostCssTransformedAsset {
    #[turbo_tasks::function]
...
#[turbo_tasks::value]
pub struct WebWorkerProcessPostCssResult {
    pub content: ResolvedVc<AssetContent>,
    pub assets: Vec<String>, // Simplified to just strings
}

#[turbo_tasks::value_impl]
impl Asset for WebWorkerPostCssTransformedAsset {
    #[turbo_tasks::function]
    async fn content(&self) -> Result<Vc<AssetContent>> {
        // Create a clone to avoid moving self
        let asset_copy = WebWorkerPostCssTransformedAsset {
            source: self.source,
            evaluate_context: self.evaluate_context,
            config_location: self.config_location,
            source_map: self.source_map,
        };
        Ok(*process_webworker_postcss(asset_copy.cell()).await?.content)
    }
}

#[turbo_tasks::function]
async fn process_webworker_postcss(
    asset: Vc<WebWorkerPostCssTransformedAsset>,
) -> Result<Vc<WebWorkerProcessPostCssResult>> {
    ...
    let FileContent::Content(content) = &*file.await? else {
        return Ok(WebWorkerProcessPostCssResult {
    ...
    Ok(WebWorkerProcessPostCssResult {
        content,
        assets: Vec::new(),
    }
    .cell())
